...
import com.lib.utils.android.vo.PhoneVo
...
object PhoneUtils {
    ...
    fun getMobilePhoneNumber(): String {
        return (Utils.instance.context.getSystemService(Context.TELEPHONY_SERVICE) as TelephonyManager).line1Number
    }
    ...
    fun dial(phoneNumber: String) {
        Utils.instance.context.startActivity(IntentUtils.getDialIntent(phoneNumber))
    }
    ...
    fun call(phoneNumber: String) {
        Utils.instance.context.startActivity(IntentUtils.getCallIntent(phoneNumber))
    }
    ...
    fun sendSms(phoneNumber: String, content: String) {
        Utils.instance.context.startActivity(IntentUtils.getSendSmsIntent(phoneNumber, content))
    }
    ...
    fun sendSmsSilent(phoneNumber: String, content: String) {
        ...
        val sentIntent = PendingIntent.getBroadcast(Utils.instance.context, 0, Intent(), 0)
    ...
    fun getAllContactInfo(): List<HashMap<String, String>> {
        ...
        try {
            ...
            while (cursor!!.moveToNext()) {
                ...
                // 判断contact_id是否为空
                if (!contact_id.isNullOrBlank()) {//null   ""
    ...
    fun getAllSMS() {
        // 1.获取短信
        // 1.1获取内容解析者
        val resolver = Utils.instance.context.getContentResolver()
