...
object FileUtils {
    ...
    fun rename(file: File?, newName: String): Boolean {
        ...
        // 新的文件名为空返回false
        if (newName.isNullOrBlank()) return false
        // 如果文件名没有改变返回true
    ...
    fun isDir(file: File?): Boolean {
        return isFileExists(file) && file?.isDirectory == true
    }
    ...
    fun isFile(file: File?): Boolean {
        return isFileExists(file) && file?.isFile == true
    }
    ...
    fun getDirSize(dir: File?): String {
        ...
        return if (len == (-1).toLong()) "" else IOUtils.byte2FitMemorySize(len)
    }
    ...
    fun getDirName(filePath: String): String {
        if (filePath.isNullOrBlank()) return filePath
